...
export const metadata = { title: "เทมเพลตทั้งหมด" };
...
export default async function TemplatesPage() {
  const list = await db()
    .select()
    .from(products)
    .where(and(eq(products.isPublished, true), eq(products.type, "template")))
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <header className="mb-6">
        ...
        <p className="text-ink/60 mt-1">ไฟล์เทมเพลตพร้อมใช้ ดาวน์โหลดได้หลังซื้อ</p>
      </header>
      {list.length === 0 ? (
