...
async function loadFont(slug: string) {
  const row = await db()
    .select()
    .from(products)
    .where(
...
export async function generateMetadata({
  ...
  const p = await loadFont(slug);
  if (!p) return { title: "ไม่พบฟอนต์" };
...
export default async function FontDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    ...
    <div className="mx-auto max-w-6xl px-4 pt-10 pb-20 grid lg:grid-cols-2 gap-10">
      <div>
        <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-cream border border-peach-100">
        ...
        {previews.length > 0 && (
          <div className="mt-3 grid grid-cols-4 gap-2">
            {previews.map((k, i) => (
              <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-cream border border-peach-100">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={thumbUrl(k)} alt={`ตัวอย่างฟอนต์ ${p.name} ${i + 1}`} loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
              </div>
            ))}
          </div>
        )}
      ...
      <div>
        ...
        <div className="mt-8 rounded-xl bg-white border border-peach-100 p-4 text-sm text-ink/70">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>ไฟล์ฟอนต์ .ttf (หรือ .otf / .zip ตามที่ร้านกำหนด)</li>
